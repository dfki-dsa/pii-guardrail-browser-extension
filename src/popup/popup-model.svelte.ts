...
export type OnboardingPromptModel = {
  ...
  open: () => void;
  /** Hide the prompt permanently without marking the flow as read. */
  dismiss: () => void;
};
  ...
  const app: AppModels = {
    ...
    onboarding: {
      ...
      open: () => {
        ...
        // Hide the prompt immediately. Completion is recorded by the flow
        // itself, so a user who opens the tour and abandons it is not marked
        // as having read it — but the popup stops offering it either way.
